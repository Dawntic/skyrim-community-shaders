...
float MarchHorizon(float2 CoordsUV, uint3 ThreadID, float SampleHeight, uint2 HeightMapPxSize, float2 Dir)
{
	...
	float StepDist = length(Dir * TexelWorldSize);  // world distance per step (handles aspect + diagonals)
	...
	[loop] for (int step = 1; step < StepCount; ++step)
	{
		...
		MaxHeight = max(MaxHeight, HDiff / Hypot);
		PosOffset += StepDist;
	}
...
#	ifdef CARDINALS
	...
	{
		if (step < 4) {
			card[step] = MarchHorizon(CoordsUV, ThreadID, HeightSample, HeightMapPxSize, CARD[step]);
		} else {
			diag[step - 4] = MarchHorizon(CoordsUV, ThreadID, HeightSample, HeightMapPxSize, DIAG[step - 4]);
		}
	}
...
#	else  // Bent Normals - outputted to 1024^2 map
	...
	[loop] for (uint step = 0; step < NUM_AZIMUTH; ++step)
	{
		...
		// 3x3 kernel: march horizon from neighbouring origins, average SinH
		float SinH = 0.0;
		float wSum = 0.0;
