...
5. ~~Ground cloud shadows~~ — done: the Cloud Shadows feature grew a Beer-Lambert shadow map for this cloud system (`CloudShadows/VolumetricCloudShadowMapCS.hlsl`). The vanilla path rasterises the sky cloud planes into a cubemap, and those planes are discarded here, so it had nothing left to read. The new map is a camera-centred square in the horizontal plane at the cloud layer base; each texel marches the shared `GetCloudProfile` field along the directional light and stores `exp(-tau)`. It is Gaussian-blurred and mip-mapped (binomial 4×4 tent per level), and receivers pick a mip from view distance, so cloud shadows do not alias into specks in the distance. `GetCloudProfile` moved into `CloudCommon.hlsli` so the view march and the shadow map integrate the same density; the cloud cbuffers moved to `PhysicalSky::UpdateCloudBuffers` (EarlyPrepass) so the map is not a frame behind the clouds casting it.
