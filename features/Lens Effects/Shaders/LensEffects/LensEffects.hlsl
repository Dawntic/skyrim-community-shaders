...
float GetWeatherFactor(float4 SunColor, float CloudFactor, float SunRadius){
    float PrecipFactor = 1.0 - LinearStep(0.5, 1.0, Precip);
...
float4 main(SunGlareVertexOutput input) : SV_Target
{
    ...
    float3 Color = WHITE * Glow - Scene + (input.Color.xyz * Edge);
...
#ifdef CHROMATIC_ABERRATION_PSSHADER
